...
test('encodes unusual characters correctly', () => {
  expect(encodeData(MEDICAL_SYMBOL.char)).toBe(MEDICAL_SYMBOL.encoded)
  expect(encodeData(NULL_CHAR.char)).toBe(NULL_CHAR.encoded)
  expect(encodeData(HIGH_UNICODE.char)).toBe(HIGH_UNICODE.encoded)
  expect(encodeData(RTL_MARK.char)).toBe(RTL_MARK.encoded)
  expect(encodeData(ZWJ.char)).toBe(ZWJ.encoded)
})

test('encodes strings with existing zero-width characters', () => {
  const encoded1 = encodeData(STRINGS.WITH_EXISTING_ZW)
  const encoded2 = encodeData(STRINGS.WITH_RTL)
  const encoded3 = encodeData(STRINGS.WITH_ZWJ)
...
test('encodes mixed script text', () => {
  const encoded = encodeData(STRINGS.MIXED_SCRIPTS)
...
test('encodes whitespace-only text', () => {
  const encoded = encodeData(STRINGS.ONLY_WHITESPACE)
...
test('encodes very long text efficiently', () => {
  const encoded = encodeData(STRINGS.VERY_LONG)
...
test('encodes text with control characters', () => {
  ...
  const encoded = encodeData(controlText)
...
test('encodes high Unicode values correctly', () => {
  const encoded = encodeData(HIGH_UNICODE.char)
...
test('encodes null character correctly', () => {
  const encoded = encodeData(NULL_CHAR.char)
